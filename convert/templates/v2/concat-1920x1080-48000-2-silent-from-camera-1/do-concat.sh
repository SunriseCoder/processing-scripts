...
for file in *; do
	...
	# MTS/MP4/MOV file -> process as video file
		...
		vid-brand-v3-only-logo.bat "$file" $video_width $video_height $audio_frame_rate $audio_channel_number $silent_mode "$file.mp4" "v2\CA_Logo_for_1920x1080_watermark_white.png"
		if ! [ $? -eq 0 ]; then
			echo exit $?
		fi
		cp "$file.mp4" concat-branded-videos
	# JPG/PNG/BMP file -> process as image file
		...
		img-to-vid-v3.bat "$file" $video_width $video_height $audio_frame_rate $audio_channel_number 5 "$file.mp4"
		if ! [ $? -eq 0 ]; then
			echo exit $?
		fi
		cp "$file.mp4" concat-branded-videos
