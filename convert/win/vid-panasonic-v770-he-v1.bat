...
set wave_name=%1.wav
set norm_wave_name=%wave_name%_norm.wav

set video_with_logo_name=%1_logo.MTS

set video_tmp_name=%1_tmp.MTS
set video_ready_name=%1_ready.MTS
...
echo ==== Wrapping Intro and Outro
	...
	echo file %resource_path_escaped%\\videos\\panasonic-v770-he\\CA_Intro_1920x1080-25fps.MTS > files_.txt
	set file_name_with_quote=%video_with_logo_name%
	set file_name=%file_name_with_quote:"=%
	echo file '%file_name%' >> files_.txt
	echo file %resource_path_escaped%\\videos\\panasonic-v770-he\\CA_Outro_1920x1080-25fps.MTS >> files_.txt
...
echo ==== Packing to mp3
	ffmpeg -i %norm_wave_name% -ar 22050 -q:a 9 %1.mp3
