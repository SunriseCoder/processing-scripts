Rem Usage: vid-brand-v3-4k <filename> <fps> [silent]
...
set silent_suffix=
if "%3"=="silent" set silent_suffix=silent
...
set intro_name=%resource_path_escaped%\\videos\\v2\\generated\\CA_Intro_%resolution%-%fps%fps-%frame_rate%Hz-%channel_number%Ch-%silent_suffix%.mp4
set outro_name=%resource_path_escaped%\\videos\\v2\\generated\\CA_Outro_%resolution%-%fps%fps-%frame_rate%Hz-%channel_number%Ch-%silent_suffix%.mp4
...
echo ==== Embedding Logo
	...
	ffmpeg ^
		...
		-c:v libx264 -crf 23 -video_track_timescale 90000 -vsync vfr -r %fps% ^
